...
import extrudePolygon from "../flat/extrudePolygon";
import Buffer from "../flat/Buffer";

export default class PlannerixBuilder {

    constructor(url, callback) {
        fetch(url)
            .then(response => response.json())
            .then(json => {
                const editorDataRaw = json.editor_data;
                const apartment = JSON.parse(editorDataRaw);

                const vertices = [];

                const outerContur = apartment.apartments[0].points;
                ...
                rootObject.add(this.generateMesh([outerContur], roomsColour, 0, holes));

                for (const room of rooms) {
                    const floorColour = new THREE.Color();
                    floorColour.set('#FF0000');
                    const points = room.points;

                    rootObject.add(this.generateMesh([points], floorColour));
    ...
    _toearcut(points, holes = undefined) {
        ...
        points.map(point => {
            earcut_coords.push(point.x);
            earcut_coords.push(point.y);
            coordIdx += 1;
        });
        ...
        if (holes !== undefined) {
            ...
            holes.map(hole => {
                ...
                hole.map(holeCoord => {
                        earcut_coords.push(holeCoord.x);
                        //coordIdx++;
                        earcut_coords.push(holeCoord.y);
                        coordIdx++;
                    }
                );
            })
